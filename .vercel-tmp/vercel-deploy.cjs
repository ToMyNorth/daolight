...
function runBuildIfNeeded(projectPath) {
  ...
  let pkgJson;
  try { pkgJson = JSON.parse(fs.readFileSync(pkgJsonPath, 'utf8')); } catch { return true; }
  if (!pkgJson.scripts || !pkgJson.scripts.build) return true;
...
function main() {
  ...
  try {
    ...
    if (result.status === 0 && output && !output.includes('Error')) log(`Logged in as: ${output}`);
  ...
  if (!fs.existsSync(absPath)) { log(`Error: Project not found: ${absPath}`); process.exit(1); }
